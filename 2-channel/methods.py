...
#################################################################
    ...
    if imageChannels == 1:   
        ...
        boxMeans = findBoxMeans(image_stack, boxSizeInPx)                # returns array of mean px value in each box; mean box value for every frame in dataset
        ...
        acfPlots=np.empty((image_stack.shape[0]*2-1))                    # empty array with otherwise the correct shape for an autocorrelation plot, will be appended to below
        ...
        summaryDict = {"Filename":name_wo_ext}                 # create dict to fill with summary stats; start with the file name
        ...
        if group_names != ['']:                                        
            summaryDict["Group Name"] = group_name               # !!!!! MAY NEED TO MODIFY THIS TO PLAY NICE WITH ANI'S GUI
        ...
        for key in summaryDict.keys(): 
            if key not in col_headers:                                # if string description of some variable is not already in the column headers list...
                col_headers.append(key)                               # ...append it
        ...
        stats_list.append(summaryDict)                             # summary of stats for this file is finished, append it to the growing list and move on to the next
...
#################################################################
    ...
    if imageChannels == 2:   
        ...
        subs = np.split(image_stack, 2, 1)                               # list object containing two arrays corresponding to the two channels of the image_stack
        ch1 = np.squeeze(subs[0],axis=1)                                # array object corresponding to channel one of image_stack. Also deletes axis 1, the "channel" axis, which is now empty
        ch2 = np.squeeze(subs[1],axis=1)                                # array object corresponding to channel two of image_stack. Also deletes axis 1, the "channel" axis, which is now empty
        ...
        columnNames = ["Parameter", "Mean", "Median", "StdDev", "SEM"]  # initial column names, will be expanded in for loop below
        Ch1AcfPlots = np.zeros((image_stack.shape[0]*2-1))               # empty array with otherwise the correct shape for an autocorrelation plot, will be appended to below
        Ch2AcfPlots = np.zeros((image_stack.shape[0]*2-1))               # empty array with otherwise the correct shape for an autocorrelation plot, will be appended to below
        ccfPlots = np.zeros((image_stack.shape[0]*2-1))                  # empty array with otherwise the correct shape for a crosscorrelation plot, will be appended to below
...
df = pd.DataFrame(stats_list, columns=col_headers)               # convert all the stats to a df
df.to_csv(directory + "/0_fileStats.csv")                               # and export as a .csv


if group_names != ['']:                                                  # if the user added group names to compare
